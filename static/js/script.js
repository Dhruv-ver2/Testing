...
function initIntroAnimation() {
    ...
    const introDuration = parseInt(getComputedStyle(overlay).getPropertyValue('--intro-duration'), 10) || 6000;
    ...
    function createFormulaParticle() {
        ...
        setTimeout(() => { if(particle.el) particle.el.style.opacity = '0.7'; }, 100);
    }
...
/*
==========================================================================
 ABOUT ME SECTION LOGIC (INTEGRATED FOR VAULT)
==========================================================================
*/
function initAboutMeSection() {
    ...
    /* UPDATED: Covers full section height (including Vault) */
    function setupAnimation() {
        ...
        canvas.height = aboutSection.scrollHeight; 
        initNodes();
        if (animationFrameId) cancelAnimationFrame(animationFrameId);
        animationFrameId = requestAnimationFrame(animateNetwork);
    }
    ...
    function wrapHeadingsInSpans() {
        document.querySelectorAll('.section-title, .project-content h3').forEach(heading => {
            ...
            words.forEach((word, wordIndex) => {
                ...
                if (wordIndex < words.length - 1) heading.append(' ');
            });
    ...
    /* UPDATED: CORRECTED MOUSE OFFSET FOR ABSOLUTE CANVAS */
    ...
    /* UPDATED: RECALCULATE HEIGHT AFTER TIMELINE EXPANDS */
    if (toggleButton && moreInfoContent) {
        toggleButton.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                // Allow CSS transition to finish then rebuild canvas height
                setTimeout(setupAnimation, 950); 
            }, 200); 
        });
    }
    ...
    if (resumeButton) {
        ...
        if (isMobile) {
            resumeButton.href = "https://www.linkedin.com/in/dhruv-vaishnav-102044375";
